...
class AudioEQService : Service() {
    ...
    fun setBandGain(bandIndex: Int, gainDb: Float) {
        ...
        activeEffects.forEach { (sessionId, effect) ->
            try {
                ...
                // Actualiza canal izquierdo (0)
                val leftBand = config.getPreEqBandByChannelIndex(0, bandIndex)
                leftBand.gain = gainDb
                config.setPreEqBandByChannelIndex(0, bandIndex, leftBand)

                // Actualiza canal derecho (1)
                val rightBand = config.getPreEqBandByChannelIndex(1, bandIndex)
                rightBand.gain = gainDb
                config.setPreEqBandByChannelIndex(1, bandIndex, rightBand)
        ...
        activeEffects.forEach { (sessionId, effect) ->
            try {
                ...
                for (b in 0 until TOTAL_PEQ_BANDS) {
                    val gain = gains[b]
                    val leftBand = config.getPreEqBandByChannelIndex(0, b)
                    leftBand.gain = gain
                    config.setPreEqBandByChannelIndex(0, b, leftBand)

                    val rightBand = config.getPreEqBandByChannelIndex(1, b)
                    rightBand.gain = gain
                    config.setPreEqBandByChannelIndex(1, b, rightBand)
                }
                effect.config = config
            } catch (e: Exception) {
    ...
    fun setLimiterEnabled(enabled: Boolean) {
        ...
        activeEffects.forEach { (sessionId, effect) ->
            try {
                val config = effect.config
                val leftLimiter = config.getLimiterByChannelIndex(0)
                leftLimiter.enabled = enabled
                config.setLimiterByChannelIndex(0, leftLimiter)

                val rightLimiter = config.getLimiterByChannelIndex(1)
                rightLimiter.enabled = enabled
                config.setLimiterByChannelIndex(1, rightLimiter)
    ...
    fun setMDRCEnabled(enabled: Boolean) {
        ...
        activeEffects.forEach { (sessionId, effect) ->
            try {
                val config = effect.config
                for (b in 0 until 3) {
                    if (config.mbcBandCount > b) {
                        val leftMbc = config.getMbcBandByChannelIndex(0, b)
                        leftMbc.enabled = enabled
                        config.setMbcBandByChannelIndex(0, b, leftMbc)

                        val rightMbc = config.getMbcBandByChannelIndex(1, b)
                        rightMbc.enabled = enabled
                        config.setMbcBandByChannelIndex(1, b, rightMbc)
                    }
                }
                effect.config = config
    ...
    fun setMDRCParameters(band: Int, threshold: Float, ratio: Float, postGain: Float) {
        ...
        activeEffects.forEach { (sessionId, effect) ->
            try {
                val config = effect.config
                if (config.mbcBandCount > band) {
                    val leftMbc = config.getMbcBandByChannelIndex(0, band)
                    leftMbc.threshold = threshold
                    leftMbc.ratio = ratio
                    leftMbc.postGain = postGain
                    config.setMbcBandByChannelIndex(0, band, leftMbc)

                    val rightMbc = config.getMbcBandByChannelIndex(1, band)
                    rightMbc.threshold = threshold
                    rightMbc.ratio = ratio
                    rightMbc.postGain = postGain
                    config.setMbcBandByChannelIndex(1, band, rightMbc)
                }
                effect.config = config
            } catch (e: Exception) {
    ...
    fun setDEQEnabled(enabled: Boolean) {
        ...
        activeEffects.forEach { (sessionId, effect) ->
            try {
                val config = effect.config
                for (b in 0 until 3) {
                    if (config.postEqBandCount > b) {
                        val leftEq = config.getPostEqBandByChannelIndex(0, b)
                        leftEq.enabled = enabled
                        config.setPostEqBandByChannelIndex(0, b, leftEq)

                        val rightEq = config.getPostEqBandByChannelIndex(1, b)
                        rightEq.enabled = enabled
                        config.setPostEqBandByChannelIndex(1, b, rightEq)
                    }
                }
                effect.config = config
