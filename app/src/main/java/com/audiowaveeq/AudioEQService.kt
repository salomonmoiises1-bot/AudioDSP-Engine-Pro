...
class AudioEQService : Service() {
        ...
        if (intent != null) {
            ...
            when (action) {
                ...
                ACTION_SET_BAND_GAIN -> {
                    ...
                    if (bandIndex in 0 until TOTAL_PEQ_BANDS) setBandGain(bandIndex, gain)
                }
                ACTION_SET_ALL_BANDS -> {
                    val gains = intent.getFloatArrayExtra(EXTRA_ALL_GAINS)
                    if (gains != null && gains.size == TOTAL_PEQ_BANDS) setAllBandGains(gains)
                }
                ACTION_SET_LIMITER_ENABLED -> {
                    setLimiterEnabled(intent.getBooleanExtra(EXTRA_LIMITER_ENABLED, true))
                }
                ACTION_TOGGLE_EQ -> {
                    toggleEQ(intent.getBooleanExtra(EXTRA_EQ_ENABLED, true))
                }
                ACTION_SET_MDRC_THRESHOLD -> {
                    setMDRCThreshold(intent.getIntExtra(EXTRA_MDRC_BAND, 0), intent.getFloatExtra(EXTRA_MDRC_THRESHOLD, -12.0f))
                }
            }
        } else {
    ...
    private fun createDynamicsProcessingConfig(): DynamicsProcessing.Config {
        ...
        val builder = DynamicsProcessing.Config.Builder(
            ...
            channelCount,
            true, TOTAL_PEQ_BANDS,
            true, 3,
            true, 3,
            true
        )

        // Frecuencias exactas estándar ISO para las 32 bandas
        val isoFrequencies = floatArrayOf(
        ...
        // Configuración iterativa limpia y compatible con el compilador en la nube
        for (i in 0 until TOTAL_PEQ_BANDS) {
            val freq = isoFrequencies[i]
        ...
        val mbcBass = DynamicsProcessing.MbcBand(isMDRCActive, 200.0f, 15.0f, 100.0f, mdrcRatios[0], mdrcThresholds[0], 4.0f, -90.0f, 1.0f, 0.0f, mdrcGains[0])
        builder.setMbcBand(0, 0, mbcBass)
        builder.setMbcBand(1, 0, mbcBass)

        val mbcMids = DynamicsProcessing.MbcBand(isMDRCActive, 3000.0f, 20.0f, 80.0f, mdrcRatios[1], mdrcThresholds[1], 3.0f, -90.0f, 1.0f, 0.0f, mdrcGains[1])
        builder.setMbcBand(0, 1, mbcMids)
        builder.setMbcBand(1, 1, mbcMids)

        val mbcHighs = DynamicsProcessing.MbcBand(isMDRCActive, 20000.0f, 10.0f, 60.0f, mdrcRatios[2], mdrcThresholds[2], 2.0f, -90.0f, 1.0f, 0.0f, mdrcGains[2])
    ...
    private fun attachOrUpdateSession(sessionId: Int, callingPackage: String) {
        try {
            if (activeEffects.containsKey(sessionId)) {
                ...
                if (existing != null && !existing.enabled) existing.enabled = true
                return
            }
            ...
            Log.e(TAG, "Error crítico al inicializar sesión $sessionId: ${e.message}", e)
        }
    }
    ...
    fun setMDRCThreshold(band: Int, threshold: Float) {
        if (band in 0..2) mdrcThresholds[band] = threshold
        activeEffects.forEach { (_, effect) ->
            try {
                if (effect.mbcBandCount > band) {
                    val leftMbc = effect.getMbcBandByChannelIndex(0, band)
    ...
    fun getCurrentBandGains(): FloatArray = currentBandGains.clone()
