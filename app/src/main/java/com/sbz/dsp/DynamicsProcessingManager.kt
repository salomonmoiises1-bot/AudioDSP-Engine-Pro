...
class DynamicsProcessingManager(
    ...
    private fun initializeEffect() {
        ...
        for (eqBands in bandAttempts) {
            try {
                ...
                Log.i(
                    TAG,
                    "Successfully initialized DynamicsProcessing for session " +
                        "$audioSessionId with $eqBands EQ bands"
                )
                ...
                Log.w(
                    TAG,
                    "Failed initializing DynamicsProcessing with $eqBands bands " +
        ...
        try {
            ...
            Log.i(
                TAG,
                "Initialized default fallback DynamicsProcessing for session $audioSessionId"
            )
        } catch (e: Exception) {
            Log.e(
                TAG,
                "FATAL: Could not initialize DynamicsProcessing for session " +
    ...
    fun applyConfig(config: DspConfig) {
        ...
        try {
            ...
            val bal = config.balance.coerceIn(
                -1.0f,
                1.0f
            )
            ...
            Log.e(
                TAG,
                "Error applying DSP config on session $audioSessionId: ${e.message}",
                e
            )
        }
    }
    ...
    private fun applyToneAndPreEq(
        ...
        for (i in 0 until bands) {
            ...
            /*
             * IMPORTANT:
             * The graphic EQ is no longer split between Pre-EQ
             * and Post-EQ for low frequencies.
             *
             * Bass Boost remains in Pre-EQ.
             * Tone remains in Pre-EQ.
             * The actual 32-band graphic EQ is applied at 100%
             * in apply32BandEq().
             */
            val bassBoostDb =
                if (
    ...
    private fun apply32BandEq(
        ...
        for (i in 0 until bands) {
            ...
            /*
             * FULL GRAPHIC EQ GAIN.
             *
             * Previously low frequencies (<= 250 Hz)
             * received only 50% here because the other 50%
             * was sent to Pre-EQ.
             *
             * That split has been removed.
             *
             * Every band, including:
             * 20, 25, 31.5, 40, 50, 63, 80,
             * 100, 125, 160, 200 and 250 Hz,
             * now receives the complete requested gain.
             */
            val gain =
                requestedGain.coerceIn(
                    -15.0f,
                    15.0f
                )
        ...
        for (
            ...
            val mbcBand =
                DynamicsProcessing.MbcBand(
                    enabled,
                    band.cutoffFrequencyHz.coerceIn(
                        20f,
                        22000f
                    ),
                    band.attackMs.coerceIn(
                        0.1f,
                        200f
                    ),
                    band.releaseMs.coerceIn(
                        10f,
                        2000f
                    ),
                    band.ratio.coerceIn(
    ...
    fun reclaimControl() {
        try {
            ...
            Log.w(
                TAG,
                "Failed reclaiming DynamicsProcessing control " +
                    "on session $audioSessionId: ${e.message}"
            )
        }
    }
    ...
    fun release() {
        try {
            ...
            Log.w(
                TAG,
                "Error releasing DynamicsProcessing on session " +
                    "$audioSessionId: ${e.message}"
            )
        } finally {
            dp = null
